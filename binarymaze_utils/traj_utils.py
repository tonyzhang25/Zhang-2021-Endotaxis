...
from binarymaze_utils.plot_utils import plot
from binarymaze_utils.maze_utils import *
import sys
from binarymaze_utils.query_data import QueryData
...
def ExtractNoseTraj(working_dir,video_name,ma,save_cont_pos=True):
    '''
    Extracts the trajectory of the nose from one processed data file.
    working_dir = local directory.
    The input data should be in <working_dir>/processed_data
    video_name = long name of the video for this processed data file
    save_cont_pos: if False then don't save the continuous keypoint data 
    ma = maze structure
    returns a new trajectory
    '''
    Data = QueryData(working_dir = working_dir)
    data = Data.get_data(video_name, save_dir = None)
    fr = np.array(data['traj indices'][video_name], dtype=int) # start and stop frame of each bout
    if 'improved discrete positions' in data:
        pos = np.array(data['improved discrete positions'][video_name]) # discrete cell (x,y) in each frame
        # extract only data during the bouts. make a list of arrays, one for each bout
        ce = [np.array([-1 if np.isnan(p[0]) else ma.ce[tuple(p)] for p in pos[f[0]:f[1]]]) for f in fr] # convert to cell numbers
    elif 'improved discrete positions (bouts)' in data:
        pos = np.array(data['improved discrete positions (bouts)'][video_name]) # discrete cell (x,y) in each frame, one list per bout
        ce = [np.array([-1 if np.isnan(xy[0]) else ma.ce[tuple(xy)] for xy in b]) for b in pos] # convert to cell numbers
    if save_cont_pos: # add the nose keypoint data
        key = np.array(data['warped numpy'][video_name])[:,0,0:2] # nose keypoint (x,y) in each frame
        ke = [key[f[0]:f[1]] for f in fr] # limit to the bouts, one array at a time
    else:
        ke = None
    no = None # Don't assign a node trajectory yet; do this after fixing errors
    re = None # Don't assign reward times yet, load those separately
    tr=Traj(fr,ce,ke,no,re)
    return tr

def ExtractRewards(working_dir,video_name,tr):
    '''
    Extracts the reward on and off frames from a processed data file.
    working_dir = local directory.
    The input data should be in <working_dir>/analysis/by_video/
    The reward times will get added to the trajectory tr
    video_name = long name of the video for this processed data file
    '''
    filename=working_dir+'/analysis/by_video/'+video_name+'.json'
    with open(filename) as json_file:
        data1 = json.load(json_file)
    rt=data1['LED reward times (trajectories)'][video_name]
    re=[]
    for b in rt: # for every bout
        c=np.array([int(x) for x in b]) # turn True/False into 1/0
        c[-1]=0 # catch problem where reward signal stays on through the end of the bout
        c[1:]-=c[:-1] # change
        fb=np.where(c==1)[0] # frame beginning reward
        fe=np.where(c==-1)[0] # frame ending reward
        re+=[np.array([fb,fe]).T] # note first dim of array is 0 if no reward
    tr.re=re[:len(tr.fr)] # add rewards truncated to the length of this trajectory
    return tr
